...
class DemoInference:
    ...
    def process_frame_sequence(self, frame_iterator, width, height, name):
        ...
        for frame_id, frame in tqdm(frame_iterator):
            ...
            if self.vis_generator and self.vis_writer:
                (vis_frame, recognition) = self.vis_generator.frame_vis_generator(orig_frame, results, width, height, name)
                #self.vis_writer.dump_artifacts(vis_frame, frame_id)

            yield frame_id, results, vis_frame, recognition
